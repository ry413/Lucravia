...
将已经跑通的 Android 订单价值分析链路逐步产品化：司机只需完成悬浮提示、位置和订单区域设置，即可在订单页看到基于 DashScope 识别与高德路线的价值分析，且绝不代替司机操作。

## Current system state

- Flutter 主界面是面向司机的启动与状态页：技术配置正常时隐藏服务细节，以“悬浮提示 → 位置服务 → 订单区域 → 开始分析”表达使用流程。
...
- 本屏多个地图路线可信的完整订单按高德有效时薪排序；最多三张有可靠坐标的订单直接以绿 / 橙 / 蓝框定位。地图匹配异常的完整订单只保留在 Flutter 详情中，不进入最佳订单或推荐框。360×124dp 最大评价窗以完整中文逐行显示排行、预计毛时薪、接驾/等客/载客耗时、接驾/载客/总行驶里程、两段红绿灯、拥堵/严重拥堵及预计道路收费；Flutter 主界面还显示主要收费道路。
...
- 完成首轮用户界面文案产品化：主页标语改为“看清时间成本，再决定抢哪单”；正常流程不再显示 VLM、局域网、`.env`、APK 等开发术语，订单区域、运行状态、通知和错误提示均使用一致的用户语言。
- 修复 Android 单应用屏幕共享冷启动目标 App 时悬浮窗偶尔不出现的时序：系统选择器打开前先预启动 `location` 前台服务，授权后再升级为 `mediaProjection|location`；悬浮窗延迟 750 ms 挂载，避开系统选择器隐藏非系统 overlay 的过渡期。
...
- 同名 POI 仍可能在宽松里程阈值内误配；当前会展示高德实际匹配名称，并把明显数量级冲突标记为 `route_mismatch`，但仍需继续用真实订单校准阈值。
