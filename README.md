...
9. 服务端将手机 GPS 转为高德坐标，直接地理编码 VLM 地址；地理编码失败时才使用附近 POI 的首个结果。当前 Mock 阶段不使用平台显示里程做同名地点消歧或结果否决；随后以高德路径规划 2.0 默认推荐策略计算“当前位置→上车点”和“上车点→目的地”两段驾车路线，并读取道路里程、实时路况分段、红绿灯、收费金额、收费里程和主要收费道路。两段路线并行处理不同订单。
